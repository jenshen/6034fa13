...
class Neuron(DifferentiableElement):
    ...
    def compute_output(self):
        ...
        returns: number (float or int)
        """
        pairs = zip(self.my_inputs, self.my_weights)
        weighted_sum = sum([i.output()*j.get_value() for (i,j) in pairs])
        return 1.0 / (1.0 + math.e**(-weighted_sum))
    ...
    def compute_doutdx(self, elem):
        ...
        if self.has_weight(elem):
            change = self.my_inputs[self.my_weights.index(elem)].output()
            return self.output() * (1 - self.output()) * change
        change = 0
        for w in self.my_weights:
            if self.isa_descendant_weight_of(elem, w):
                change += self.my_inputs[self.my_weights.index(w)].dOutdX(elem) * w.get_value()
        return self.output() * (1 - self.output()) * change
